...
Optcontrol::Optcontrol(bool isPWD, DMatrix &Q, Function &h, DVector &refVec,
...
DVector
Optcontrol::solveOptimalControl(DVector & NewRefVec) {

    static double t{ 0. };
    static std::clock_t previousTime{ std::clock() };

    if (abs(NewRefVec[0] - this->refVec(0)) > 1.) {
        if (NewRefVec[0] > this->refVec(0)) {
            NewRefVec[0] = this->refVec(0) + 1.;
        } else {
            NewRefVec[0] = this->refVec(0) - 1.;
        }
    }
    ...
    if (abs(NewRefVec[1] - this->refVec(1)) > 1.) {
        if (NewRefVec[1] > this->refVec(1)) {
            NewRefVec[1] = this->refVec(1) + 1.;
        } else {
            NewRefVec[1] = this->refVec(1) - 1.;
        }
    }
    ...
    if (abs(NewRefVec[2] - this->refVec(2)) > 1.) {
        if (NewRefVec[2] > this->refVec(2)) {
            NewRefVec[2] = this->refVec(2) + 1.;
        } else {
            NewRefVec[2] = this->refVec(2) - 1.;
        }
    }
    ...
    referenceVG.setVector(0, refVec);
...
Function Optcontrol::getFunction() {
    return h;
}
...
void Optcontrol::setrefVec(DVector & newrefVec) {
    refVec = newrefVec;
}
